export function toDecimal(deg, min, sec, dir) {
...
export function addMarker(location, label, map) {
...
export function addMarker2(location, label, map) {
    var marker = new google.maps.Marker({
        position: location,
        label: label,
        map: map,
...
export function addPath(path, pathColor, map) {
    var polywidth = 1;
...
export function addPath2(path, pathColor, map) {
    var polywidth = 10;
    return new google.maps.Polyline({
        path: path,
        map: map,
...
export function addSector(path, sectorColor, map) {
...
export function TMA(center, radius, map) {
...
export function drawArc(center, initialBearing, finalBearing, radius) {
    ...
    for (let i = 0; i < points; i++) {
        extp.push(destPt(wp, initialBearing + i * deltaBearing, radius));
