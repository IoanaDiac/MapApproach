import { addSector, toDecimal, addPath, addMarker, addPath2 } from "./drawingUtils.js";
import { pointsLoc, waypoints, vorNdb, STARS } from "./points.js";
import { choice, dist, choiceWind, viewActive } from "./star_choice.js";
import { getAirportsFromAPI, latlng, store } from "./get_airport.js";
import { paths, clearPaths4 } from "./paths.js";
import { clearPaths } from "./activeStars.js";
export var map;
export let txt = document.createElement('p');
document.body.appendChild(txt);
function init() {
    //MARKERS
    map = new google.maps.Map(document.getElementById("map"), { center: { lat: toDecimal(44, 21, 44, "N"), lng: toDecimal(28, 29, 18, "E") }, zoom: 9 });
    ...
    //POLYGON
    let sector1 = addSector([[]], "#0cff00", map);
    for (let i = 0; i <= 32; i++) {
    ...
    //RWY
    let path2 = addPath2([], "#808080", map);
    path2.getPath().push(pts[33]);
    path2.getPath().push(pts[34]);
    /**
...
&offset=1000&limit=500`);
    ...
    console.log(store);
    const arp = document.getElementById("airport");
    /**
...
//console.log(airport[0].getName());
    ...
    sub.addEventListener("click", function (event) {
        ...
        if (arp.value == "" && windText.value == "") {
            ...
            txt.textContent = "Please introduce your Departure Airport";
        else if (windText.value == "")
            txt.textContent = 'Introduce a wind direction between 0 and 360';
        else if (parseFloat(windText.value) < 0 || parseFloat(windText.value) > 360)
            txt.textContent = 'Introduce a wind direction between 0 and 360';
        else {
            // console.log(windDir);
            let airport = store.getAirports();
            //console.log(airport[0].getName());
            for (let i = 0; i < airport.length; i++) {
                if ((airport[i].getIcao() == arp.value) || (airport[i].getName() == arp.value) || (airport[i].getPosition() == latlng(arp.value))) {
                    mrk = addMarker(airport[i].getPosition(), airport[i].icao, map);
